...
let ``Running utility from PATH``() =
    let exitCode = ExecuteCommand.runCmdLocally (fun x -> printfn "%s" x) "path utility test" "dotnet" "--list-runtimes" Environment.CurrentDirectory
    ...
    member s.``Running local executable``() =
        let localExecutable =
            if isTestRuntimeWindows then
                ...
                "copy_prog"
        let wd = Path.GetFullPath(Path.Combine(s.RelativeExperimentRoot,"../../")) // this is "data" dir.
        ...
        let newLocation = Path.GetFullPath(Path.Combine(s.RelativeExperimentRoot,"texturalData.txt"))
        ...
        let exitCode = ExecuteCommand.runCmdLocally (fun x -> System.Diagnostics.Trace.WriteLine(sprintf "%s" x)) "local executable test" localExecutable args wd
        Assert.Equal(0,exitCode)